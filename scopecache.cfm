<!---
	...
	scope: 			The scope where cached data will reside. Must be either session, 
					application, server, or request. (required)
...
<cfif not isDefined("attributes.scope") or not isSimpleValue(attributes.scope) or not listFindNoCase("application,session,server,request",attributes.scope)>
	<cfthrow message="scopeCache: The scope attribute must be passed as one of: application, session, server, or request.">
</cfif>
...
<cfif isDefined("attributes.r_cacheItems")>
